...
on a.product_id=b.product_id --joining the product id column from both tables
group by a.userid
...
6.which item was first purchased by the customer after they became a member?
 ...
 9.if buying each product generates points for eg 5rs=2 zomato point and each product has different purchasing points for eg for 
 p1 5rs=1 zomato point, for  p2 10rs=5zomato point and p3 5rs=1 zomato. 
 ...
 10.  In the first  one year after  a customer joins the gold program(including their join date) irrespective of what the 
      customer has purchased  they earn 5 zomato points for every 10 rs spent who earned more  more 1 or 3 and  what was the
	  their points earings in their first year?
	  
	  1 zomato points=2rs
	  0.5 zomato points=1rs
 ...
 12.rank all the transctions for each member whenever they are a zomato gold member for every non gold member transaction mark as na?
 ...
 -- cast is used to change the data type